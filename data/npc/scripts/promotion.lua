...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'brawler') and talkState[talkUser] == 1) then
		if (firstPromotion(cid, 2) == true) then
			...
			setPlayerStorageValue(cid, 15903, 2)
		else
	...
	elseif(msgcontains(msg, 'help')) then
		selfSay('There are 3 tiers of promotions. The first tier costs 5k, which include 4 vocations. Second tier costs 100k, which include 8 vocations. Type \'promotion\' to obtain one of these vocations.', cid)
	else
		selfSay('Your choices are {brawler}, {ranger}, {squire}, and {shaman}.', cid)
	end
	return true
end
